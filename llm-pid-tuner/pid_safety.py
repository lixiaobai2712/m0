...
def build_fallback_suggestion(
    ...
    if status in {"LINE_LOST", "EDGE_RISK"}:
        proposal["p"] *= 0.85
        proposal["i"] *= 0.80
        proposal["d"] *= 0.90
        action         = "REDUCE_LINE_LOSS_RISK"
        summary        = "Line loss or edge risk detected; reducing steering aggression."
...
def score_metrics(metrics: Dict[str, float]) -> float:
    ...
    avg_error          = float(metrics.get("avg_error", 1e9))
    steady_state_error = float(metrics.get("steady_state_error", 1e9))
    overshoot          = float(metrics.get("overshoot", 1e9))
    status             = str(metrics.get("status", "UNKNOWN")).upper()

    if metrics.get("control_domain") == "line_follow":
        lost_ratio = float(metrics.get("lost_ratio", 0.0) or 0.0)
        edge_ratio = float(metrics.get("edge_ratio", 0.0) or 0.0)
        crossing_ratio = float(metrics.get("crossing_ratio", 0.0) or 0.0)
        reversal_ratio = float(metrics.get("reversal_ratio", 0.0) or 0.0)
        return (
            avg_error + steady_state_error * 0.8
            + lost_ratio * 10000.0 + edge_ratio * 1200.0
            + crossing_ratio * 300.0 + reversal_ratio * 150.0
        )
